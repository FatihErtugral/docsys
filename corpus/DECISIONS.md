...
| D-026 | **Universal adoption of an existing agent layer.** docsys assets use never-colliding names (`commands/docsys-sync.md`, `skills/docsys/`); the AGENTS.md block lives between `docsys:rules` markers and is updated idempotently, owner prose untouched; `agents --report` mechanically inventories the existing layer and the shell commands it invokes. Detection is the tool's; deciding what to delegate stays judgment — the tool never rewrites an owner's file (first t-embed integration was hand surgery; this decision makes it a supported flow). |
