...
fn apply_moves_rewrites_and_scaffolds() {
    ...
    assert!(
        a.contains("[b](../howto/b.md)"),
        "in-tree link must follow the move: {a}"
    );
    assert!(
        a.contains("[out](../../outside.md)"),
        "out-of-tree link must be depth-corrected: {a}"
    );
...
fn apply_rewrites_inbound_repo_references() {
    ...
    assert!(
        readme.contains("example.com/x/docs/howto/guide.md"),
        "{readme}"
    );
