...
fn product_composes_verbatim_with_shifted_headings_and_stamps() {
    ...
    .unwrap();
    let done = docsys::export::product(&root, &map, None).unwrap();
    let out = &done.output;
...
fn product_refuses_to_half_compose() {
    ...
    .unwrap();
    let err = docsys::export::product(&root, &map, None).unwrap_err();
...
fn declared_languages_are_read_never_determined() {
    ...
    let done = docsys::export::product(&root, &map, Some("en")).unwrap();
    assert!(
        done.warnings
            .iter()
...
fn feature_composes_a_slice_and_follow_widens_one_hop() {
    ...
    let done =
        docsys::export::feature(&root, &["token-ttl".to_string()], false, None, None).unwrap();
...
fn unchanged_output_leaves_the_file_untouched() {
    ...
    let two = docsys::export::feature(&root, &["token-ttl".to_string()], true, None, None).unwrap();
...
fn plan_drafts_a_map_grouped_by_type() {
    ...
    let draft = docsys::export::plan(&root).unwrap();
